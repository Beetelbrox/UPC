...
int main( int argc, char *argv[] ) {
    ...
    gpuErrchk(cudaMalloc(&dev_u, np*np*sizeof(float)) );
    gpuErrchk(cudaMalloc(&dev_uhelp, np*np*sizeof(float)) );
    gpuErrchk(cudaMalloc(&res_cuda, Grid_Dim*Grid_Dim*sizeof(float)) ); 
    res_vector = (float*)malloc(Grid_Dim*Grid_Dim*sizeof(float));
    ...
    iter = 0;
    while(1) {
        gpu_Heat<<<Grid,Block>>>(dev_u, dev_uhelp, np, res_cuda);
        gpuErrchk(cudaPeekAtLastError());
        gpuErrchk(cudaMemcpy(param.u, dev_u, np*np*sizeof(float), cudaMemcpyDeviceToHost));
        cudaThreadSynchronize();                        // wait for all threads to complete

        // We are now computing the residual in the GPU, so we have commented out the old statements to copy the data back to the CPU.
        ...
        cudaMemcpy(res_vector, res_cuda, sizeof(float)*Grid_Dim*Grid_Dim, cudaMemcpyDeviceToHost);
	
        residual = 0.0;
        cudaThreadSynchronize();
        int i;
	for(i=0; i < Grid_Dim*Grid_Dim; i++)
          residual += res_vector[i];
