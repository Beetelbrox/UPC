...
```
```{r}
table(dep_tree_metric_seqs[[1]]$n)
```

```{r, echo=FALSE}
# Generate s factor
lapply( seq(from=1, to=length(dep_tree_metric_seqs), by=1), function(i) {
  lapply( seq(from=1, to=length(nl_models[[i]]), by=1), function(j) {
...
for (i in seq(from=1, to=length(dep_tree_metric_seqs), by=1)) {
  ...
  lines(x=dep_tree_metric_seqs[[i]]$n,
        y=functions_ensemble[[1]]()(dep_tree_metric_seqs[[i]]$n),
        col="red"
  )
}
```
