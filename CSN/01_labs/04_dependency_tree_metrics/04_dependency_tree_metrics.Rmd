  ...
  lapply( seqs, function(seq) {         # Outer loop to iterate over the dependency sequences
    ...
    lapply(ensemble, function(model) {  # Inner loop to iterate over the models
      if (length(model$params) > 0) {
        ...
        nls( formula=model$formula, data=seq, start=initial_values, control=nls.control(maxiter=5000), trace=TRUE)
      } else NA
    })
  })
}
...
test_seqs <- lapply(test_functions, function(f) data.frame(
  ...
  d=sapply(sapply(1:num_test_observations, function(i) f(i)) + rnorm(num_test_observations, sd=0.05), function(x) if (x>=1) x else 1))) # The nls freaks out w/ values close to 0 so we force all values to be at least 1 (after all, the min distance is 1  )
```
