...
nl_models <- lapply( dep_tree_metric_seqs, function(seq) {
  ...
  lapply( seq(from=1, to=length(log_formulas), by=1), function(i) {
    if ( length(params_ensemble[[i]]) > 0 ) {
      lm <- lm(log_formulas[[i]], seq)
      # Inner loop 2 to iterate over the parameters of each model
      initial_values <- structure(sapply( seq(from=1, to=length(params_ensemble[[i]]), by=1), function(j)
      ...
      # Generate the non-linear model
      nls( formula=formula_ensemble[[i]], data=seq, start=initial_values, trace=FALSE )
    } else { NA }
  })
})
```
...
# Generate AICs
AIC_deviances <- lapply( seq(from=1, to=length(dep_tree_metric_seqs), by=1), function(i) {
  sapply( seq(from=1, to=length(nl_models[[i]]), by=1), function(j) {
    if ( length(params_ensemble[[j]]) > 0 ) {
      AIC(nl_models[[i]][[j]]) }
    else {
...
```
```{r}
best_model <- lapply(AIC_deviances, function(x) which.min(as.vector(x)))
...
for (i in seq(from=1, to=length(dep_tree_metric_seqs), by=1)) {
  ...
  lines(x=avg_seqs[[i]]$n,
        y=do.call (functions_ensemble[[ as.numeric(best_model[i]) ]], as.list(best_coefs[[i]]))(avg_seqs[[i]]$n),
        col="red"
  )
}
