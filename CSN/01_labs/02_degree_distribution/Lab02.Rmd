...
\item We also found an unexpected difference between the catalan and the arabic languages. We did not include the basque in this comparison, because it was one of the regions that resisted the arabic invasion, and also because the origin of the basque language is uncertain. Taking into account the establishment of the arabic-muslim empire during the Middle Age in most iberian territory (almost seven centuries), we were expecting a big similarity between these languages. But they differ in almost every degree property as well as for the AIC values. 
\end{itemize}
