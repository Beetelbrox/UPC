---
title: "Complex & Social Networks: Lab 02 Report"
author:
  - Francisco Javier Jurado Moreno
  - Sergio Mosquera Dopico
output: pdf_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

```{r cars, include=FALSE}
# Load the igraph library and require the packages stats4 (mle) & VGAM (Riemman-zeta)
library(igraph)
require('stats4')
require('VGAM')
```

```{r}
# Read the list of languages from file
languages = read.table("list.txt",
              header = TRUE,               # this is to indicate the first line of the file contains the names of the columns instead of the real data
              as.is = c("language","file") # this is need to have the cells treated as real strings and not as categorial data.
            )

# Read the language files from disk and store them in memory so we don't have to do so multiple times
langs_in_degree <- vector('list', nrow(languages))
for (i in seq(from=1, to=nrow(languages))){
  langs_in_degree[i] <- read.table(languages$file[i], header = FALSE)
}
```

```{r}
# Function to write the languages summary (Table 1) from the already-read language data
write_summary <- function(lang_name, lang_in_degree) {
   # Generate the language summary (corresponding to a row of table 1) by concatenating the following fields (Language name must be accessed)
   # | language | num nodes | max degree | mean degree | inv mean degree |
   cat(lang_name,
       length(lang_in_degree),
       max(lang_in_degree),
       sum(lang_in_degree)/length(lang_in_degree),
       length(lang_in_degree)/sum(lang_in_degree),
       "\n")
}
```

```{r}
# Generate table 1 by iterating through all languages
for (i in seq(from=1, to=nrow(languages))){
  write_summary(languages$language[i],langs_in_degree[[i]])
}

```

```{r pressure, echo=FALSE}
x <- degree_sequence$V1
N <- length(x)
M <- sum(x)
M_prime <- sum(log(x))
C <- sum(sapply(seq(from=1, to=N, by=1), function(i) sum(sapply(seq(from=1, to=x[i], by=1), function(j) log(j)))))
```

```{r}
calculate_mle <- function(degree_sequence) {
  N <- length(degree_sequence)
  M <- sum(degree_sequence)
  M_prime <- sum(log(degree_sequence))
  C <- sum(sapply(seq(from=1, to=N, by=1), function(i) sum(sapply(seq(from=1, to=degree_sequence[i], by=1), function(j) log(j)))))
  cat(N,M,M_prime,C)
}
```

```{r}
calculate_mle(langs_in_degree[[1]])
```

```{r}
minus_log_likelihood_zeta <- function(gamma) {
   M_prime*gamma + N*log(zeta(gamma))
}

minus_log_likelihood_poisson <- function (lambda) {
   N*(lambda + log(1 - exp(-lambda))) + C - M*log(lambda)
}

minus_log_likelihood_geometric <- function (q) {
  (N - M)*log(1-q) - N*log(q)
}

minus_log_likelihood_zeta_gamma_2 <- 2*M_prime + N*log((pi^2)/6)

gen_harmonic <- function(k_max, gamma) {

}

minus_log_likelihood_rtrunc_zeta <- function (gamma, k_max) {
  gamma*M_prime + N*log(sum(sapply(seq(from=1, to=k_max, by=1), function(x) x^(-gamma))))
}
```

```{r}

for (lang in languages) {
  x <- degree_sequence$V1
  N <- length(x)
  M <- sum(x)
  M_prime <- sum(log(x))
  C <- sum(sapply(seq(from=1, to=N, by=1), function(i) sum(sapply(seq(from=1, to=x[i], by=1), function(j) log(j)))))
  mle_poisson <- mle(minus_log_likelihood_poisson,
                      start = list(lambda = M/N),
                      method = "L-BFGS-B",
                      lower = c(1.0000001))

  lambda <- attributes(summary(mle_poisson))$coef[1]

  mle_geometric <- mle(minus_log_likelihood_geometric,
                       start = list(q = N/M),
                       method = "L-BFGS-B",
                       lower = c(0.0000001),
                       upper = c(0.9999999)) # We needed to set an upper bound

  q <- attributes(summary(mle_geometric))$coef[1]

  mle_zeta <- mle(minus_log_likelihood_zeta,
                  start = list(gamma = 2),
                  method = "L-BFGS-B",
                  lower = c(1.0000001))

  zeta <- attributes(summary(mle_zeta))$coef[1]

  mle_rtrunc_zeta <- mle(minus_log_likelihood_rtrunc_zeta,
                  start = list(gamma = 2, k_max = max(x)), #We've chosen the largest k
                  method = "L-BFGS-B") #

  rtrunc_zeta <- attributes(summary(mle_rtrunc_zeta))$coef[1]
  k_max <- attributes(summary(mle_rtrunc_zeta))$coef[2]
}

```

```{r}
k_max
```

```{r}
max(x)
```

```{r}
for (i in 1:nrow(languages)) {
  mle_poisson <- mle(minus_log_likelihood_poisson,
                     start = list(lambda = M/N),
                     method = "L-BFGS-B",
                     lower = c(0.0000001))
  print(attributes(summary(mle_zeta))$coef[1])
  }
```

Note that the `echo = FALSE` parameter was added to the code chunk to prevent printing of the R code that generated the plot.
